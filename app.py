...
#loading the model_info from the JSON file
with open('model_info.json', 'r') as json_file:
    model_info_json = json_file.read()

model_info = json.loads(model_info_json)

#loading the model from the saved path
loaded_model = joblib.load(model_info['model_path'])
...
def main():
    ...
    x = st.number_input("x", min_value=0.0, max_value=10.0, step=0.1)
    y = st.number_input("y", min_value=0.0, max_value=10.0, step=0.1)
    z = st.number_input("z", min_value=0.0, max_value=10.0, step=0.1)
